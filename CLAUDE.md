...
pnpm run hooks                                        # one-time: register the pre-push git hook
pnpm test                                             # lint + knip + plugin validate + all Cucumber specs
pnpm run coverage                                     # same but wrapped in c8 (coverage table)
...
pnpm exec cucumber-js --name "some scenario title"   # run matching scenarios by name
pnpm pack --dry-run                                   # verify published tarball contents
```
